# Hailuo 2.3 API 日本語ガイド：モデル ID・秒単価・実行例
...
## APIMart 経由で Hailuo 2.3 を呼ぶ理由

- **1 つのキーで全カタログ。** 同じベース URL と認証ヘッダで Hailuo 2.3 を含む 300 以上の画像・動画・言語モデルに到達でき、切り替えは `model` フィールドのみ。
...
curl --request POST \
  ...
  --data '{"model":"MiniMax-Hailuo-2.3-Fast", "prompt":"a cozy reading nook by a rainy window, warm lamp light", "n":1}'
```
